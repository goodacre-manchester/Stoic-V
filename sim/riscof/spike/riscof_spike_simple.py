...
class spike_simple(pluginTemplate):
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       # runTests function
       # Vendored fix: use the riscv64 multilib toolchain (the Ubuntu/Xilinx
       # one) targeting rv32 via -march/-mabi (no riscv32- prefix here), and pin
       # the full in-scope -march (per-test isa strings like 'rv32izbb' lack the
       # underscore GCC needs). The DUT compiles identically, so both run the
       # same binary — the comparison is hardware-vs-Spike execution.
       self.compile_cmd = 'riscv64-unknown-elf-gcc -march=rv32im_zba_zbb_zbs \
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # Vendored fix: the substring checks above match the 'V' in the leading
      # "RV32", spuriously enabling vector ('rv32imv...') which Spike rejects.
      # Pin Spike to the in-scope ISA (zicsr is on by default in Spike).
      self.isa = 'rv32im_zba_zbb_zbs'
      ...
      # Vendored fix: riscv-config >=3.x normalizes PMP to a scalar and may omit
      # the legacy 'PMP' node entirely (our M-mode core has no PMP). Default the
      # granularity to 4 bytes rather than KeyError on ispec['PMP'].
      _pmp = ispec.get('PMP', {})
      if isinstance(_pmp, dict) and 'pmp-grain' in _pmp:
          # convert from G to bytes: g = 2^(G+2) bytes
          self.granularity = pow(2, _pmp['pmp-grain'] + 2)
      else:
...
#          cmd = self.compile_cmd.format(testentry['isa'].lower().replace('zicsr', ' ', 2), self.xlen, test, elf, compile_macros)
      ...
      # echo statement.
          if self.target_run:
            # Vendored fix: Spike >=1.1.x removed '--misaligned' (now default);
            # the extra --log-commits run was debug-only. Just dump the signature.
